...
const LogisticsDashboard: React.FC<{ housings: Housing[], cars: CarType[] }> = ({ housings, cars }) => {
...
const ReportDamageModal: React.FC<{ car: CarType, onClose: () => void, onReport: (desc: string) => void }> = ({ car, onClose, onReport }) => {
...
const OperationsTab: React.FC = () => {
  ...
  const housingMapInstance = useRef<L.Map | null>(null);
  ...
  useEffect(() => {
    ...
    if (activeSubTab === 'housing' && viewMode === 'map' && housingMapRef.current && !housingMapInstance.current) {
  ...
  }, [activeSubTab, viewMode, filteredHousing, smartZoneId]);
