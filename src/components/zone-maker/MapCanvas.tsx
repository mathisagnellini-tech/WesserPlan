...
const MapCanvas: React.FC<MapCanvasProps> = ({
  ...
  // Wait for the container to actually have dimensions before initialising
  // Leaflet — otherwise tiles never paint and the map stays grey. Previous
  // implementation set up a ResizeObserver but returned from the effect
  // (with deps []), so the map never initialised on a 0×0 container.
  useEffect(() => {
  ...
  // Init map once the container is ready. Tracks tile layer + cleans up on
  // unmount (window listeners + L.Map removal).
  useEffect(() => {
    if (!containerReady || !mapContainerRef.current || mapInstanceRef.current) return;
  ...
  useEffect(() => {
    ...
    const geoJsonLayer = L.geoJSON(features as unknown as GeoJSON.FeatureCollection, {
      style: (feature) => {
        if (!feature) return {};
      ...
      onEachFeature: (feature, layer) => {
        const props = feature.properties as { communeId: string; isAssigned: boolean; clusterId?: string; communeData: Commune };
        const handleBrushAction = () => {
          if (!isBrushMode) return;
          const cid = props.communeId;
          if (props.isAssigned) return;
        ...
        layer.on({
          ...
          click: (e) => {
            L.DomEvent.stopPropagation(e);
            if (isBrushMode) return;
            if (onCommuneClick) onCommuneClick(props.communeId);
            if (props.clusterId) {
              const cluster = clusters.find((c) => c.id === props.clusterId);
    ...
    if (visibleTeamPath !== null && visibleTeamPath !== undefined && routeLayerRef.current) {
      ...
      if (pathPoints.length > 1) {
        ...
        for (let i = 0; i < pathPoints.length - 1; i++) {
          ...
          // Angle is numeric — safe. Step content is a glyph constant.
          L.marker(mid, {
            icon: L.divIcon({
    ...
    clusters.filter((c) => c.code).forEach((cluster) => {
      ...
      // Cluster code can come from Supabase (clusters.code) — escape it
      // before injecting into the divIcon HTML. Cluster colour is gated
