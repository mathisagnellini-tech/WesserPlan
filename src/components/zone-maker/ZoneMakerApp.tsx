...
const App: React.FC = () => {
  ...
  if (error) return <div className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400"><p>{error}</p></div>;
        ...
        <div className="fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                    ...
                    <button onClick={() => setShowSettings(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><X size={24} /></button>
                </div>
                <div className="p-6 overflow-y-auto space-y-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                           {COMMUNE_STATUSES.map(status => (
                               <label key={status.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-orange-300 hover:bg-orange-50/50 dark:hover:bg-orange-900/20 cursor-pointer transition-all">
                                   <input type="checkbox" checked={statusFilters.has(status.id as CommuneStatus)} onChange={() => toggleStatusFilter(status.id)} className="w-4 h-4 rounded text-orange-600 focus:ring-orange-500 border-slate-300 dark:border-slate-600"/>
                    ...
                    <div>
                        ...
                        <input type="range" min="10000" max="200000" step="5000" value={maxPopFilter} onChange={(e) => setMaxPopFilter(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-slate-900"/>
                    </div>
                    <div>
                ...
                <div className="p-5 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                    <button onClick={() => setShowSettings(false)} className="px-4 py-2 rounded-lg font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">Fermer</button>
                    {/* Fix: removed 3 === 4 intentional comparison which was always false */}
                    <button onClick={() => { setShowSettings(false); handleGenerate(); }} className="bg-slate-900 dark:bg-orange-600 hover:bg-black dark:hover:bg-orange-700 text-white px-5 py-2 rounded-lg font-medium shadow-lg shadow-slate-200 dark:shadow-slate-900 transition-all active:scale-95">Appliquer et Regénérer</button>
                </div>
            </div>
        </div>
      )}
          ...
          <div className="fixed inset-0 bg-black/60 z-[1100] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 flex gap-3 justify-end">
                  <button onClick={() => setPendingScheduleChange(null)} className="px-4 py-2 rounded-lg text-slate-600 dark:text-slate-400 font-medium hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">Annuler</button>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-md overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-6 space-y-6">
                    <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                        Vous souhaitez ajouter <strong className="text-slate-900 dark:text-white">{pendingMove.communeName}</strong> ({pendingMove.communePop.toLocaleString()} habitants) à la zone <strong className="text-emerald-700">{pendingMove.targetClusterCode}</strong>.
                        <br/><br/>
                    ...
                    <div className="relative pt-2">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-xl p-4 text-center">
                                <div className="text-xs font-bold text-red-800 dark:text-red-300 uppercase tracking-wider mb-2">Zone {pendingMove.sourceClusterCode}</div>
                                <div className="text-2xl font-bold text-red-900 dark:text-red-200">{pendingMove.impact.source.newPop.toLocaleString()}</div>
                                <div className="text-xs text-red-600 dark:text-red-400 mt-1">{pendingMove.impact.source.oldWeeks} sem. &rarr; <strong>{pendingMove.impact.source.newWeeks} sem.</strong></div>
                            </div>
                            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/30 rounded-xl p-4 text-center">
                                <div className="text-xs font-bold text-emerald-800 dark:text-emerald-300 uppercase tracking-wider mb-2">Zone {pendingMove.targetClusterCode}</div>
                                <div className="text-2xl font-bold text-emerald-900 dark:text-emerald-200">{pendingMove.impact.target.newPop.toLocaleString()}</div>
                                <div className="text-xs text-emerald-600 dark:text-emerald-400 mt-1">{pendingMove.impact.target.oldWeeks} sem. &rarr; <strong>{pendingMove.impact.target.newWeeks} sem.</strong></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 flex gap-3 justify-end">
                    <button onClick={() => setPendingMove(null)} className="px-4 py-2 rounded-lg text-slate-600 dark:text-slate-400 font-medium hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">Non</button>
      ...
      <aside className="w-full md:w-1/3 lg:w-[420px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex flex-col h-screen overflow-hidden sticky top-0 shadow-xl z-30">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex-none bg-white dark:bg-slate-900">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowSettings(true)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 dark:text-slate-400 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-700" title="Paramètres & Filtres"><Settings size={20} /></button>
          </div>
          <div className="space-y-5 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
             <div className="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="range" min="1" max="5" step="1" value={defaultTeamCount} onChange={(e) => setDefaultTeamCount(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-orange-600"/>
                </div>
                <div>
                    ...
                    <input type="range" min="4000" max="20000" step="1000" value={targetPop} onChange={(e) => setTargetPop(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-orange-600"/>
                </div>
             </div>
             <div className="flex gap-2">
                 ...
                 <button onClick={handleExportCSV} className="px-3 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Exporter CSV">
                    <Download size={18}/>
                 </button>
             </div>
          </div>
              ...
              <div key={slot.week} className="grid gap-3 items-stretch text-sm group/week" style={{ gridTemplateColumns: `50px repeat(${maxCapacity}, 1fr)` }}>
                ...
                <div className="flex flex-col items-center justify-start py-1 border-r border-slate-200/50 dark:border-slate-700/50">
                    ...
                    <div className="flex flex-col items-center gap-0.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md shadow-sm">
                        <button onClick={() => modifyWeekTeamCount(slot.week, 1)} className="text-slate-400 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 w-full flex justify-center py-0.5 rounded-t"><Plus size={10} /></button>
                        <div className="text-[10px] font-bold text-slate-700 dark:text-slate-300 px-1">{slot.capacity}</div>
                        <button onClick={() => modifyWeekTeamCount(slot.week, -1)} className="text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 w-full flex justify-center py-0.5 rounded-b"><Minus size={10} /></button>
                    </div>
                </div>
              ...
              <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 z-10 sticky top-0 flex-none">
                  <div className="flex items-start justify-between flex-wrap gap-4">
                     ...
                     <div className="flex items-center gap-2">
                        <button onClick={() => setIsEditMode(!isEditMode)} className={`p-2.5 rounded-xl transition-all font-bold text-xs flex items-center gap-2 ${isEditMode ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800/30 shadow-inner' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm'}`}><Edit3 size={16} />{isEditMode ? 'Mode Édition' : 'Modifier'}</button>
                        ...
                        <button onClick={() => setSelectedCluster(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400"><X size={20} /></button>
                     </div>
                  </div>
                            ...
                            {selectedCluster.communes.sort((a,b) => b.population - a.population).map(c => {
                                ...
                                return (
                                    <tr key={c.id} className="group hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition-colors">
                                        ...
                                        <td className="px-4 py-2.5 text-right"><button onClick={(e) => { e.stopPropagation(); handleExcludeCommune(c.id); }} className="text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-1.5 rounded transition-colors opacity-0 group-hover:opacity-100" title="Exclure de la liste"><Trash2 size={14} /></button></td>
                                    </tr>
                                );
                            })}
                        </tbody>
                ...
                <div className="flex-1 max-w-sm relative">
                    ...
                    {searchResults.length > 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-[500]">
                            {searchResults.map(c => (
                                <button key={c.id} onClick={() => { setFocusedCommuneId(c.id); setSearchQuery(c.name); }} className="w-full text-left px-4 py-2 hover:bg-orange-50 dark:hover:bg-orange-900/20 text-sm flex justify-between items-center group">
