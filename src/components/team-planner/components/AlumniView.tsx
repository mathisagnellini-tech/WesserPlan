...
const StatCard: React.FC<{ label: string; value: string | number; trend?: string; icon: React.ElementType; color: string }> = ({ label, value, trend, icon: Icon, color }) => (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] flex items-start justify-between group hover:border-slate-300 dark:hover:border-slate-600 transition-all">
        <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">{label}</p>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{value}</h3>
            {trend && <div className="flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-400 mt-1"><TrendingUp size={10} /> {trend}</div>}
        </div>
...
const SegmentedControl: React.FC<{ options: { id: string; label: string; count?: number }[]; active: string; onChange: (id: any) => void }> = ({ options, active, onChange }) => (
    <div className="flex p-1 bg-slate-200/50 dark:bg-slate-800 rounded-xl w-fit backdrop-blur-sm border border-slate-200/50 dark:border-slate-700">
        {options.map((opt) => {
            ...
            return (
                <button
                    ...
                    className={`
                        ...
                        ${isActive
                            ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm scale-[1.02] ring-1 ring-black/5 dark:ring-white/10'
                ...
                >
                    {opt.label}
                    {opt.count !== undefined && (
                        <span className={`px-1.5 py-0.5 rounded-full text-[9px] ${isActive ? 'bg-slate-100 dark:bg-slate-600 text-slate-600 dark:text-slate-300' : 'bg-slate-300/50 dark:bg-slate-700 text-slate-500 dark:text-slate-400'}`}>
                            {opt.count}
                        </span>
                    )}
                </button>
...
export const AlumniView: React.FC<AlumniViewProps> = ({ alumni, onInfoClick, onAddToIncoming }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#F8FAFC] dark:bg-slate-950">
        {/* --- HEADER DASHBOARD --- */}
        <div className="pt-8 pb-6 px-8 border-b border-slate-200/60 dark:border-slate-700/60 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl sticky top-0 z-30">
            <div className="max-w-[1600px] mx-auto">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter mb-2 flex items-center gap-3">
                            Alumni Network
                            <span className="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest translate-y-0.5">
                                v2.4
                            </span>
                        </h1>
                        <p className="text-slate-500 dark:text-slate-400 font-medium text-sm max-w-lg leading-relaxed">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="pl-10 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-200 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all w-64 shadow-sm placeholder:text-slate-400 dark:placeholder:text-slate-500"
                            />
                        </div>
                        <button className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:shadow-md transition-all active:scale-95">
                            <Filter size={18} />
                        </button>
                    </div>
                </div>
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        Trier par: <span className="text-slate-700 dark:text-slate-300 cursor-pointer hover:underline">Date de retour</span>
                ...
                {filteredAlumni.length === 0 && (
                    <div className="h-64 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl mt-8">
