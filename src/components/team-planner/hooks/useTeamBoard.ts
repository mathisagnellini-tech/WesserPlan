...
export function useTeamBoard(options: UseTeamBoardOptions = {}) {
  ...
  useEffect(() => {
    ...
    teamPlannerService
      ...
      .then((result) => {
        ...
        // else: leave whatever's already in weeksData (initialData fallback for week 0,
        // or whatever was generated by handleDuplicateWeek for other weeks)
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  const retrySave = useCallback(() => {
    triggerSave(weeksData[currentWeekIndex] ?? initialData, currentWeekIndex);
