        ...
        <div className="fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 className="font-bold text-lg flex items-center gap-2 text-slate-900"><Settings size={20} className="text-blue-600"/> Paramètres & Filtres</h3>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                           {COMMUNE_STATUSES.map(status => (
                               <label key={status.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 cursor-pointer transition-all">
                                   <input type="checkbox" checked={statusFilters.has(status.id as CommuneStatus)} onChange={() => toggleStatusFilter(status.id)} className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-slate-300"/>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                 <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center gap-4">
                     <div className="bg-blue-200 text-blue-800 font-bold p-3 rounded-lg text-lg text-center min-w-[60px]">
                     ...
                     <div>
                         ...
                         <div className="flex items-center gap-2 text-lg font-medium text-slate-900">
                             ...
                             <span className="font-bold text-blue-700">{pendingScheduleChange.newDuration} sem.</span>
                         </div>
                     </div>
                 </div>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
                  ...
                  <button onClick={confirmScheduleChange} className="px-5 py-2 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 shadow-md flex items-center gap-2">Confirmer</button>
              </div>
            </div>
          </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-[1100] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-md overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
                    ...
                    <button onClick={confirmMove} className="px-5 py-2 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 shadow-md flex items-center gap-2">Oui</button>
                </div>
            </div>
        </div>
      )}
      ...
      <aside className="w-full md:w-1/3 lg:w-[420px] bg-white border-r border-slate-200 flex flex-col h-screen overflow-hidden sticky top-0 shadow-xl z-30">
        <div className="p-6 border-b border-slate-100 flex-none bg-white">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2 tracking-tight"><div className="bg-blue-600 text-white p-1.5 rounded-lg"><MapPin size={20} /></div>ZonePlanner</h1>
          ...
          <div className="space-y-5 bg-slate-50 p-4 rounded-xl border border-slate-100">
             <div className="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="range" min="1" max="5" step="1" value={defaultTeamCount} onChange={(e) => setDefaultTeamCount(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/>
                </div>
                <div>
                    ...
                    <input type="range" min="4000" max="20000" step="1000" value={targetPop} onChange={(e) => setTargetPop(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/>
                </div>
             </div>
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50/50 pb-40">
          <div className="flex justify-between items-center px-2">
             <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2"><Calendar size={14} className="text-blue-500"/> Calendrier & Équipes</h2>
              ...
              <div key={slot.week} className="grid gap-3 items-stretch text-sm group/week" style={{ gridTemplateColumns: `50px repeat(${maxCapacity}, 1fr)` }}>
                ...
                <div className="flex flex-col items-center justify-start py-1 border-r border-slate-200/50">
                    ...
                    <div className="flex flex-col items-center gap-0.5 bg-white border border-slate-200 rounded-md shadow-sm">
                        <button onClick={() => modifyWeekTeamCount(slot.week, 1)} className="text-slate-400 hover:text-blue-600 hover:bg-blue-50 w-full flex justify-center py-0.5 rounded-t"><Plus size={10} /></button>
                        ...
                        <div key={idx} onClick={() => handleClusterSelect(cluster)} style={{ gridRow: `span ${rowSpan}` }}
                          className={`
                            ...
                            ${selectedCluster?.id === cluster.id 
                                ? 'bg-slate-800 border-slate-900 text-white shadow-lg ring-2 ring-blue-500/50 scale-[1.02] z-20' 
                                : 'bg-white border-slate-200 text-slate-700 hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 z-10'}
                          `}
                        >
            ...
            <>
              <div className="p-4 sm:p-5 border-b border-slate-100 bg-white z-10 sticky top-0 flex-none">
                  <div className="flex items-start justify-between flex-wrap gap-4">
                     <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            ...
                            <span className="text-xs bg-blue-50 text-blue-700 px-2.5 py-1 rounded-md font-bold border border-blue-100 flex items-center gap-1"><Clock size={12}/> {selectedCluster.durationWeeks} semaine{selectedCluster.durationWeeks > 1 ? 's' : ''}</span>
                        </div>
                     </div>
                     <div className="flex items-center gap-2">
                        ...
                        {!analysis && (<button onClick={handleAnalyze} disabled={isAnalyzing} className="p-2.5 rounded-xl bg-gradient-to-br from-purple-600 to-indigo-600 text-white hover:shadow-lg hover:shadow-purple-200 transition-all active:scale-95" title="Analyser avec IA">{isAnalyzing ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}</button>)}
                  ...
                  <div className="mt-4 flex items-center gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-2">
                           <button onClick={() => handleUpdateDuration(selectedCluster.id, -1)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 transition-colors" title="Réduire durée"><Minus size={14}/></button>
                           <span className="font-mono font-bold text-slate-700 w-8 text-center">{selectedCluster.durationWeeks}s</span>
                           <button onClick={() => handleUpdateDuration(selectedCluster.id, 1)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 transition-colors" title="Augmenter durée"><Plus size={14}/></button>
                      </div>
            ...
            <div className="bg-white/95 backdrop-blur-md shadow-lg border border-slate-200/50 rounded-2xl p-2 px-6 flex items-center gap-6 pointer-events-auto max-w-4xl w-full justify-between">
                ...
                <div className="flex-1 max-w-sm relative">
                    ...
                    <input 
                       ...
                       className="w-full pl-10 pr-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:bg-white transition-all"
                    />
                    {searchResults.length > 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-[500]">
                            {searchResults.map(c => (
                                <button key={c.id} onClick={() => { setFocusedCommuneId(c.id); setSearchQuery(c.name); }} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm flex justify-between items-center group">
                                    <span className="font-medium text-slate-700">{c.name}</span>
                                    <span className="text-xs text-slate-400 font-mono group-hover:text-blue-500">{c.population}</span>
                                </button>
                            ))}
                        </div>
                    )}
            ...
            {!isLoading && filteredCommunes.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-100 z-10">
                    <div className="text-center max-w-md p-8">
                        ...
                        <button onClick={() => setShowSettings(true)} className="mt-4 text-blue-600 font-bold hover:underline">Ouvrir les paramètres</button>
                    </div>
                </div>
            )}
