...
const OperationsTab: React.FC<OperationsTabProps> = ({ isActive }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    const [housingsRes, carsRes] = await Promise.all([
      supabase.from('housings').select('*').order('created_at', { ascending: false }),
  ...
  return (
    <div className="min-h-screen pb-10 relative">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            <div className="flex gap-1 bg-slate-100 p-1.5 rounded-xl w-full sm:w-fit border border-slate-200">
