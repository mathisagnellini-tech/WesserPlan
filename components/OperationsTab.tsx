                ...
                <div className="p-6">
                    ...
                    {step === 1 && (
                        <div className="space-y-4">
                             <div className="bg-slate-50 dark:bg-slate-800/50 p-8 border-2 border-dashed border-[var(--border-subtle)] rounded-xl flex flex-col items-center justify-center text-[var(--text-muted)] hover:border-blue-400 hover:bg-blue-50 transition-colors cursor-pointer group">
                             ...
                             <div>
                                 ...
                                 <textarea 
                                    className="w-full border border-[var(--border-subtle)] rounded-lg p-3 text-sm focus:ring-2 focus:ring-orange-500 outline-none"
                                    rows={3}
...
const AddHousingModal: React.FC<{ 
    ...
    return ( <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in"> <div className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity" onClick={onClose}></div> <div className="relative bg-white dark:bg-[var(--bg-card-solid)] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]"> <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-b border-[var(--border-subtle)] flex justify-between items-center"> <h3 className="font-bold text-lg text-[var(--text-primary)]">{mode === 'manual' ? 'Saisie Manuelle' : 'Scanner un document'}</h3> <button onClick={onClose}><X className="text-[var(--text-muted)] hover:text-slate-600" /></button> </div> <div className="p-6 overflow-y-auto"> {mode === 'scan' && ( <div className="flex flex-col items-center justify-center py-8 text-center"> {scanStatus === 'idle' && ( <div onClick={handleScanFile} className="w-full h-48 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 hover:bg-purple-50 transition-colors"> <Upload size={48} className="text-slate-300 mb-4" /> <p className="font-medium text-[var(--text-secondary)]">Cliquez pour uploader le reçu</p> <p className="text-xs text-[var(--text-muted)] mt-1">PDF, JPG, PNG acceptés</p> </div> )} {scanStatus === 'scanning' && ( <div className="py-12"> <Loader2 size={48} className="text-purple-600 animate-spin mb-4 mx-auto" /> <p className="font-bold text-[var(--text-primary)]">Analyse du document...</p> <p className="text-sm text-[var(--text-secondary)]">Extraction des données via IA</p> </div> )} {scanStatus === 'done' && ( <div className="py-12 text-green-600"> <CheckCircle2 size={48} className="mx-auto mb-4" /> <p className="font-bold">Analyse terminée !</p> </div> )} <button onClick={() => setMode('manual')} className="mt-6 text-blue-600 text-sm hover:underline font-medium">Passer à la saisie manuelle</button> </div> )} {mode === 'manual' && ( <form onSubmit={handleManualSubmit} className="space-y-4"> <div className="grid grid-cols-2 gap-4"> <div className="col-span-2"> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Nom du Logement</label> <input type="text" className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" placeholder="Ex: Gîte des Lilas" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} autoFocus required /> </div> </div> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Adresse complète</label> <div className="relative"> <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" size={16} /> <input type="text" className="w-full pl-9 pr-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" placeholder="Ex: 12 Rue de la Paix, 75000 Paris" value={formData.address || ''} onChange={e => setFormData({...formData, address: e.target.value})} required /> </div> </div> <div className="grid grid-cols-2 gap-4"> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Date début location</label> <input type="date" className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" value={formData.date || ''} onChange={e => setFormData({...formData, date: e.target.value})} required /> {formData.date && ( <p className="text-xs text-blue-600 font-bold mt-1 text-right">{getWeekNumberLabel(formData.date)}</p> )} </div> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Capacité (Pers.)</label> <div className="flex items-center gap-2"> <input type="number" className="w-16 px-2 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium text-center" value={formData.people || ''} onChange={e => setFormData({...formData, people: +e.target.value})} /> <button type="button" onClick={() => setFormData({...formData, people: 5})} className={`px-3 py-2 rounded-lg text-xs font-bold border ${formData.people === 5 ? 'bg-blue-100 border-blue-200 text-blue-700' : 'bg-white dark:bg-[var(--bg-card-solid)] border-[var(--border-subtle)] text-[var(--text-secondary)] hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>5</button> <button type="button" onClick={() => setFormData({...formData, people: 10})} className={`px-3 py-2 rounded-lg text-xs font-bold border ${formData.people === 10 ? 'bg-blue-100 border-blue-200 text-blue-700' : 'bg-white dark:bg-[var(--bg-card-solid)] border-[var(--border-subtle)] text-[var(--text-secondary)] hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>10</button> </div> </div> </div> <div className="grid grid-cols-2 gap-4"> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Coût Total (€)</label> <input type="number" className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" placeholder="0" value={formData.cost || ''} onChange={e => setFormData({...formData, cost: +e.target.value})} required /> </div> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Source</label> <select className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium bg-white dark:bg-[var(--bg-card-solid)]" value={formData.channel || 'Direct'} onChange={e => setFormData({...formData, channel: e.target.value})} > <option value="Direct">Direct</option> <option value="Airbnb">Airbnb</option> <option value="Booking">Booking</option> <option value="Gîtes">Gîtes</option> </select> </div> </div> <div className="grid grid-cols-2 gap-4"> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Nom Contact</label> <input type="text" className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" placeholder="M. Martin" value={formData.ownerName || ''} onChange={e => setFormData({...formData, ownerName: e.target.value})} /> </div> <div> <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase mb-1">Téléphone</label> <input type="tel" className="w-full px-3 py-2 border border-[var(--border-subtle)] rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium" placeholder="06..." value={formData.owner || ''} onChange={e => setFormData({...formData, owner: e.target.value})} /> </div> </div> <div className="flex gap-3 pt-2"> <button type="button" onClick={onClose} className="flex-1 py-3 text-[var(--text-secondary)] font-bold text-sm hover:bg-slate-100 rounded-lg transition-colors">Annuler</button> <button type="submit" className="flex-1 py-3 bg-blue-600 text-white font-bold text-sm rounded-lg shadow-md hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"> <Save size={16} /> Enregistrer </button> </div> </form> )} </div> </div> </div> );
};

const HousingDetailModal: React.FC<{ housing: Housing | null; onClose: () => void; }> = ({ housing, onClose }) => {
    if (!housing) return null;
    return ( <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in"> <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={onClose}></div> <div className="relative bg-white dark:bg-[var(--bg-card-solid)] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]"> <div className="h-40 bg-slate-100 dark:bg-slate-800 relative"> <div className="absolute inset-0 flex items-center justify-center text-slate-300"> <MapPin size={48} opacity={0.2}/> <span className="ml-2 text-sm font-medium">Aperçu carte indisponible</span> </div> <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-white/80 dark:bg-[var(--bg-card-solid)]/80 hover:bg-white rounded-full text-[var(--text-secondary)] transition-colors shadow-sm"> <X size={20} /> </button> <div className="absolute bottom-4 left-6"> <span className={`px-3 py-1 rounded-lg text-xs font-bold text-white shadow-sm ${housing.org === 'MSF' ? 'bg-red-600' : housing.org === 'UNICEF' ? 'bg-blue-500' : 'bg-green-600'}`}> Utilisé par {housing.org} </span> </div> </div> <div className="p-8 overflow-y-auto"> <div className="flex justify-between items-start mb-6"> <div> <h2 className="text-2xl font-bold text-[var(--text-primary)] leading-tight mb-1">{housing.name}</h2> <p className="text-[var(--text-secondary)] text-sm">{housing.address}</p> <div className="flex items-center gap-2 mt-1"> <span className="text-xs font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded">{getWeekNumberLabel(housing.date)}</span> <span className="text-xs text-[var(--text-secondary)]">{housing.region} ({housing.dept})</span> </div> </div> <div className="text-right"> <div className="text-2xl font-extrabold text-[var(--text-primary)]">{housing.cost} € <span className="text-sm font-medium text-[var(--text-muted)]">/ sem.</span></div> <div className="text-sm text-[var(--text-secondary)]">{(housing.cost / housing.nights).toFixed(0)}€ / nuit</div> </div> </div> <div className="grid grid-cols-3 gap-4 mb-8"> <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-[var(--border-subtle)] flex flex-col items-center justify-center text-center"> <User className="text-blue-500 mb-2" size={24} /> <span className="font-bold text-[var(--text-primary)] text-lg">{housing.people}</span> <span className="text-xs text-[var(--text-secondary)] uppercase font-bold">Personnes</span> </div> <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-[var(--border-subtle)] flex flex-col items-center justify-center text-center"> <Bed className="text-blue-500 mb-2" size={24} /> <span className="font-bold text-[var(--text-primary)] text-lg">{housing.nights}</span> <span className="text-xs text-[var(--text-secondary)] uppercase font-bold">Nuits min.</span> </div> <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-[var(--border-subtle)] flex flex-col items-center justify-center text-center"> <Star className="text-yellow-500 mb-2" size={24} fill="currentColor" /> <span className="font-bold text-[var(--text-primary)] text-lg">{housing.rating}/5</span> <span className="text-xs text-[var(--text-secondary)] uppercase font-bold">Avis Équipe</span> </div> </div> <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8"> <div> <h3 className="font-bold text-[var(--text-primary)] mb-3">Commentaire Chef d'Équipe</h3> <div className="bg-yellow-50 p-4 rounded-xl text-sm text-[var(--text-primary)] italic border border-yellow-100 relative"> <span className="absolute top-2 left-2 text-4xl text-yellow-200 font-serif leading-none">"</span> <p className="relative z-10">{housing.comment || "Aucun commentaire spécifique."}</p> <p className="text-right mt-2 text-xs font-bold not-italic text-[var(--text-muted)]">- {housing.lead}</p> </div> </div> <div> <h3 className="font-bold text-[var(--text-primary)] mb-3">Commodités</h3> <div className="space-y-2"> {housing.amenities.map(am => ( <div key={am} className="flex items-center gap-3 text-sm text-[var(--text-secondary)]"> <CheckCircle2 size={16} className="text-green-500"/> {am} </div> ))} </div> </div> </div> <div className="border-t border-[var(--border-subtle)] pt-6 flex items-center justify-between"> <div className="flex items-center gap-3"> <div className="w-10 h-10 bg-slate-200 dark:bg-slate-700 rounded-full flex items-center justify-center text-[var(--text-secondary)]"> <User size={20} /> </div> <div> <p className="text-sm font-bold text-[var(--text-primary)]">{housing.ownerName}</p> <p className="text-xs text-[var(--text-secondary)]">{housing.channel}</p> </div> </div> <button className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 flex items-center gap-2"> <Phone size={18} /> {housing.owner} </button> </div> </div> </div> </div> );
};
    ...
    <div className="min-h-screen pb-10 relative">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full sm:w-fit border border-[var(--border-subtle)]">
                ...
                <button onClick={() => setActiveSubTab('cars')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeSubTab === 'cars' ? 'bg-white dark:bg-[var(--bg-card-solid)] text-blue-600 shadow-sm' : 'text-[var(--text-secondary)] hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
                    🚗 Véhicules & Flotte
                </button>
            </div>
            ...
            <div className="animate-fade-in space-y-6">
                ...
                <div className="bg-white dark:bg-[var(--bg-card-solid)] p-4 rounded-2xl border border-[var(--border-subtle)] shadow-sm flex flex-col md:flex-row gap-4 items-center">
                    <div className="flex-grow">
                        ...
                        <select 
                            className="w-full bg-slate-50 dark:bg-slate-800/50 border border-[var(--border-subtle)] text-[var(--text-primary)] font-bold rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                            value={smartZoneId}
                    ...
                    {smartZoneId && (
                        <div className="flex items-center gap-4 px-4 py-2 bg-blue-50 rounded-xl border border-blue-100 text-sm text-blue-800 animate-fade-in">
                            ...
                            <button onClick={() => setSmartZoneId("")} className="bg-white dark:bg-[var(--bg-card-solid)] p-1 rounded-full hover:bg-blue-100 transition"><X size={14}/></button>
                        </div>
                    )}
                    <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-[var(--border-subtle)] h-fit">
                        <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-[var(--bg-card-solid)] text-blue-600 shadow-sm' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`} title="Liste"><ListIcon size={20}/></button>
                        <button onClick={() => setViewMode('map')} className={`p-2 rounded-lg transition-colors ${viewMode === 'map' ? 'bg-white dark:bg-[var(--bg-card-solid)] text-blue-600 shadow-sm' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`} title="Carte"><MapPin size={20}/></button>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in">
                        {filteredHousing.map((h: any) => (
                            ...
                            >
                                    ...
                                    {/* Card Footer */}
                                    <div className="p-4 border-t border-[var(--border-subtle)] flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/30 group-hover:bg-blue-50/30 transition-colors">
                                            <div>
